...
import { post,findById,getAll,comment,updateComment,deleteComment,likePost,likeComment, replyComment } from "../modules/annonymous/annon";
...
export const CreatePost = async (req:Request,res:Response,next:NextFunction)=>{
    try {
        ...
        const response = await post(userId,content,image?.path);
...
export const LikePost = async (req:Request,res:Response,next:NextFunction)=>{
    try {
        ...
        const response = await likePost(parseInt(postId),userId);
        if(response.error){
            next(response.error);
        };
        if(response.success == false){
